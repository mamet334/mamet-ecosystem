...
const healerEndStr = "[Execution Contract] VALIDATED OK. Starting execution loop.`;\n      }";

const healerStart = content.indexOf(healerStartStr);
let healerEnd = content.indexOf(healerEndStr);
if (healerEnd === -1) {
    // Try without template literal backticks in case git transformed something
    healerEnd = content.indexOf("VALIDATED OK. Starting execution loop");
    // Find the next closing brace after this
    healerEnd = content.indexOf("}", healerEnd) + 1;
}
...
if (healerStart !== -1 && healerEnd !== -1) {
    ...
    content = content.substring(0, healerStart) + healerReplacement + content.substring(healerEnd);
