...
15. **Dekomposisi Penuh `engineer.js` (2978 Baris) — [`ADR-0017-engineer-js-decomposition.md`](../adr/ADR-0017-engineer-js-decomposition.md):**
    ...
    - **Progres:** Fase 1-7 selesai — `engineer.js` 2978 → **1261 baris** (−1717, ~58%). Fase 7 (risiko tertinggi sejauh ini, sengaja digabung) mengimplementasikan `CodeSnippetExtractor.js` §2.1 dari nol — brace-matching string-aware yang melacak stack konteks (string/template literal/interpolasi/komentar) agar `{`/`}` literal di dalam template literal (persis pola `_buildPatchPrompt` sendiri) tidak salah dihitung sebagai pembuka/penutup blok kode. Percabangan lama berbasis jumlah-karakter (4000 awal+4000 akhir untuk file >6000 char) dihapus total, diganti pemangkasan presisi berbasis identifier target dari task; format output ke LLM sekarang selalu search-replace untuk semua ukuran file. Diverifikasi ketiga jalur ekstraksi (`full-file`/`identifier`/`keyword-scan`) termasuk kasus sulit interpolasi bersarang, plus `buildPatchPrompt` end-to-end dan `_generatePatch` wrapper terhadap instance Engineer sungguhan. Selanjutnya: Fase 8 (`PatchApplier.js` + orchestrator final, terakhir & risiko tertinggi).
    - **Terhubung dengan Item Lain:** Fase 7 ini **sengaja digabung** dengan implementasi `CodeSnippetExtractor.js` dari `SPESIFIKASI-TEKNIS-MAMET-OS-v2.md` §2.1 (Backlog terkait §2.1 di dokumen itu — sekarang **selesai diimplementasikan**, bukan lagi cuma desain) — keduanya menyentuh `_buildPatchPrompt()` yang sama, dikerjakan sekali agar tidak ada dua PR tumpang tindih.
