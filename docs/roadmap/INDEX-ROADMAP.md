...
73. **Aturan Kutipan Persis untuk Dokumen RAG (2026-09-12):**
    ...
    - **Dicatat, belum dikerjakan:** catat `data.provider` di baris `[PR#6 TOKEN METRICS]` (satu baris, gratis) agar kejadian seperti di atas bisa dibuktikan; bila terbukti, kunci daftar penyedia lewat parameter `provider` pada permintaan OpenRouter.
      → **Dikerjakan di Item 75** (2026-09-13): `penyedia=` kini tercatat; terbukti satu model dilayani penyedia berganti-ganti.

74. **Ikon dan Huruf Disimpan Sendiri — Lepas dari Google Fonts (2026-09-13):**
    - **Asal:** Owner mengirim tangkapan layar sidebar yang berubah menjadi daftar kata (`architecture`, `home`, `chat_bubble`, `terminal`) disertai log `GET https://fonts.googleapis.com/… net::ERR_TIMED_OUT` (dua tautan: Material Symbols dan Inter/Geist/JetBrains Mono).
    - **Akar masalah:** ikon ditulis sebagai LIGATUR (`<span class="material-symbols-outlined">home</span>`) dan deklarasi `font-family` untuk kelas itu TIDAK PERNAH ada di kode kita — datang dari CSS Google. `index.css:70` hanya mengatur `font-variation-settings` + `vertical-align`. CSS Google gagal → tidak ada `font-family` sama sekali → yang tampil katanya. Ikut terungkap: stylesheet yang timeout MENAHAN tampilnya halaman, dan di Mamet Desktop tanpa internet hal ini SELALU terjadi. Timeout-nya berulang, bukan sekali lewat — dicoba dari mesin lain keesokan harinya `fonts.googleapis.com` tetap `ConnectTimeoutError`.
    - **Perbaikan (commit `7706cd3` ikon, `411e253` huruf):**
      - `src/assets/fonts/material-symbols-subset.woff2` — **21 KB / 67 ikon** (font penuh 970 KB, diukur) dan `inter-latin-wght-normal.woff2` — **47 KB**, satu berkas variabel 100–900, potongan latin.
      - `index.html`: kedua `<link>` Google dihapus; **CSP dipersempit** — `style-src` tak lagi mengizinkan `fonts.googleapis.com`, `font-src` tinggal `'self'`.
      - **Aturan `.material-symbols-outlined` ditulis di dalam `@layer base`.** Kelas Google memuat `font-size: 24px` sedangkan banyak ikon memakai `text-[13px]`/`text-[16px]`; dulu urutan terjaga karena CSS Google di `<link>` terpisah, sementara aturan biasa di `index.css` ditulis SESUDAH `@tailwind utilities` dan akan mengalahkan kelas ukuran. Tanpa `@layer base`, semua ikon membesar.
      - `font-display`: **block** untuk ikon (yang "tampil apa adanya" berupa kata), **swap** untuk teks.
      - Perintah pemeliharaan: `npm run ikon` (pindai `src/` → unduh ulang subset; `daftar-ikon.txt` ikut disimpan agar terlihat di diff) dan `npm run huruf` (versi dipatok, sumber Fontsource/jsDelivr karena Google tak terjangkau bahkan untuk mengunduh).
    - **Uji sebelum deploy** (server dev, memakai `index.css` sebenarnya, diukur lewat DOM): 67 ikon semuanya **selebar 24 px** (ligatur jadi satu glyph; yang gagal akan menampilkan katanya dan jauh lebih lebar); `text-[13px]/[16px]/[32px]` → 13/16/32 px (utilities tetap menang); `document.fonts` mencatat tepat dua muka (`Inter 100 900 loaded`, `Material Symbols Outlined 100 700 loaded`); lebar kalimat contoh 445 px (Inter 400) vs 455 px (Inter 700) vs 428 px (huruf sistem); daftar permintaan jaringan seluruhnya localhost. Build produksi: kedua woff2 terbit ber-hash, `dist/index.html` tanpa `<link>` luar, aturan ikon di posisi 4.765 sedangkan utilities ukuran di 59.852+.
    - **Status:** ✅ **Selesai, Dideploy & Terbukti di Produksi** (mamet-ecosystem.vercel.app, 13 September 2026):
      - Halaman hanya memuat SATU stylesheet — `assets/index-nktrmgy6.css`, nama ber-hash yang sama persis dengan build lokal, jadi yang live memang build ini.
      - `performance.getEntriesByType('resource')`: **nol** permintaan ke googleapis/gstatic.
      - `inter-latin-wght-normal-C2S99t-D.woff2` (47 KB) diambil dari origin sendiri; `document.fonts` mencatat `Inter 100 900 loaded`.
      - Font ikon awalnya berstatus `unloaded` — **bukan gagal**: halaman masuk tidak memuat satu pun elemen `.material-symbols-outlined` (diperiksa: daftar elemennya kosong), sehingga browser belum perlu mengunduhnya. Dipaksa lewat `document.fonts.load()`, berkasnya turun 21 KB dari origin sendiri dan statusnya menjadi siap.
      - Catatan: pemeriksaan pertama tepat sesudah Owner men-deploy masih menampilkan build lama (`index-Knuo9gK5.css`) — deploy Vercel belum selesai; diperiksa ulang sampai bundel baru muncul.
      - **Dikonfirmasi Owner (13 September 2026):** sesudah masuk, ikon tampil sebagai gambar di versi web — bahkan sebelum aplikasi selesai memuat. Sesuai harapan: font kini berkas sendiri 21 KB, bukan stylesheet luar yang menahan tampilan; sisa pemuatan adalah bundel JavaScript dan data Supabase, bukan huruf.
    - **Yang dikorbankan (keputusan sadar):** tautan yang dicabut membawa TIGA huruf. **Geist** (12 tempat: `font-display-lg`, `font-headline-md`) dan **JetBrains Mono** (6 tempat: `font-label-mono`) tidak ikut diunduh → huruf sistem. Cadangan Geist diarahkan ke Inter di `tailwind.config.js` agar judul tetap serumpun, bukan jatuh ke Arial/Segoe UI. Bila ingin dikembalikan persis: Geist 27,7 KB + JetBrains Mono 39,5 KB, barisnya sudah disiapkan sebagai komentar di `scripts/perbarui-huruf.mjs`.
    - **Keterbatasan yang disadari:**
      - **Ikon baru tidak otomatis masuk.** Subset hanya berisi nama yang terpindai; nama yang dirakit saat berjalan harus ditambah manual di `TAMBAHAN_MANUAL`. Yang terlewat tampil sebagai kata — hanya ikon itu saja.
      - **Belum membuat aplikasi jalan offline.** Yang dijamin ikon dan huruf; offline penuh versi web butuh service worker (Fase 4 Item 72).
      - `mametlite/` diperiksa dan memang tidak pernah memuat Google Fonts — tidak tersentuh.

75. **Nalar yang Benar-Benar Mati, Penyedia Hulu Tercatat, dan Jam Lokal sebagai Data Sistem (2026-09-13):**
    - **Asal:** satu pertanyaan uji — "melihat status jam saat ini" — memperlihatkan tiga masalah sekaligus, plus butir tertunda Item 73. Changelog: [`2026-09-13-nalar-mati-penyedia-dan-jam-lokal.md`](../project-memory/changelog/2026-09-13-nalar-mati-penyedia-dan-jam-lokal.md).
    - **(a) Penyedia hulu OpenRouter dicatat (commit `03a1318`):** field `provider` di respons dan tiap chunk stream dulu dibuang, sehingga dugaan Item 73 ("penyedia berbeda") tak bisa dibuktikan. Kini `penyedia=<nama>` di baris `[PR#6 TOKEN METRICS] OpenRouter`; `processOpenAIStream` menerima `info` opsional, pemanggil Groq/OpenAI tak berubah. Uji 16/16. **Produksi:** satu model, penyedia berganti — 13:03 OpenInference, 13:22 Relace, 13:36 OpenInference.
    - **(b) Thinking tiga keadaan (commit `b1008ca`):** tier SEDANG yang Thinking-nya TIDAK dicentang tetap bernalar **1.491 token** dan jawaban selesai 37 detik kemudian. Sebab bertumpuk: frontend mengirim `thinking: aiThinking || undefined` (nilai false hilang, server memakai `=== true`), dan adapter sengaja tak pernah mengirim "mati" — padahal dokumentasi OpenRouter (dicek 2026-09-13) menyatakan model yang mampu bernalar MENYALAKANNYA OTOMATIS bila parameter tak dikirim. Kini: true → `enabled: true`; **false → `enabled: false`**; undefined (mis. mametlite) → tak dikirim. `lib/adapters/reasoning_openrouter.ts` (baru): model yang menolak dimatikan (400 menyebut "reasoning", terbukti `gemini-3.5-flash-lite` di Item 67) diulang sekali tanpa parameter lalu diingat selama instans hidup. Groq/OpenAI/Gemini tak berubah. Uji 12/12.

      | | 13:03 (sebelum) | 13:22 (sesudah) | 13:36 (sesudah) |
      |---|---|---|---|
      | Token nalar | 1.491 | **0** | **0** |
      | Token jawaban | 1.804 | 113 | 59 |
      | Lama menunggu model | 37 s | 2,6 s | 3,0 s |

      Biaya sengaja tidak dibandingkan: token ter-cache antar chat 3.544 / 0 / 3.266, jadi selisihnya bercampur efek cache. ⚠️ **Tier THINKING Owner juga berstatus Thinking tidak dicentang** — dulu tetap bernalar karena "mati" tak pernah terkirim, kini benar-benar tidak bernalar kecuali dicentang di Settings.
    - **(c) Data sistem bukan dasar VERIFIED (commit `7e0cb68`):** jawaban jam berlabel `VERIFIED` karena tabel zona waktu ebook (skor 0,573) dikutip untuk sebagian isinya; tabel itu waktu STANDAR, sehingga Dublin (seharusnya UTC+1) dan Bahamas (UTC−4) meleset satu jam karena musim panas. `label_sumber.ts` meloloskannya karena judul dokumen benar — ia memeriksa kutipan, bukan asal fakta. Kini stempel waktu header ditulis "Waktu server (DATA SISTEM, bukan dokumen)" dan BLOK 6 melarang data sistem jadi Sumber/dasar VERIFIED; inti dari data sistem atau pengetahuan sendiri → HYPOTHESIS. Tanpa label baru. Uji 10/10. **Produksi 13:22:** `HYPOTHESIS`, "Sumber: Data sistem (waktu server), bukan dari dokumen RAG", tanpa koreksi otomatis.
    - **(d) Browser mengirim zona waktu (commit `c3b3a1d`):** setelah (c) model berhenti menebak tapi juga berhenti membantu. Log membongkar bahwa chat 13:03 yang menjawab "20:03 WIB untuk Anda di Indonesia" pun TIDAK tahu — riwayat 0 pesan, tanpa memori; model menebak dari bahasa pertanyaan dan baris pertama tabel ebook. Kini AssistantService (LOOKUP + percakapan) mengirim `clientTimezone` dari `Intl`; `lib/request/waktu_pengguna.ts` (baru) memvalidasi nama zona dan menulis KONTEKS WAKTU bertanda DATA SISTEM (jam lokal, zona, selisih UTC, tanggal lokal; musim panas dihitung `Intl`), menggantikan tanggal UTC + tahun "2026" yang ditulis tetap; log `[Waktu] Zona waktu pengguna`. Uji 26/26 (Dublin Sep UTC+01:00 / Jan UTC+00:00, Nassau UTC−04:00, tanggal melewati tengah malam, sisipan baris baru & kutip ditolak).
    - **Status:** ✅ **Selesai, Dideploy & Terbukti di Produksi.** Bukti (d), chat baru 13:36:08 UTC: `[Waktu] Zona waktu pengguna: Asia/Jakarta`, `riwayat=0 pesan/0 huruf`, jawaban "Minggu, 13 September 2026, pukul 20.36 WIB" — tepat, tanpa riwayat dan tanpa tebakan; prompt identitas +333 huruf.
    - **Keterbatasan yang disadari:**
      - Jam yang dihitung benar tetap berlabel `HYPOTHESIS - Rekomendasi AI` — bukan rekomendasi, tapi belum ada label untuk jawaban dari data sistem.
...
76. **Tabel DOCX Dibaca sebagai Tabel, dan Riset Jalur PDF untuk RAG (2026-09-14):**
    ...
    - **(b) Jalur PDF — detektor + OCR (commit `4604ccf`, diperbaiki `de75807`, 14 September 2026):**
      heuristik B′ (`deteksiTabelHalaman`) menandai halaman bertabel dari posisi teks pdf.js;
      `pdfOcrService.js` (frontend & mametlite) memotong halaman itu lewat `pdf-lib` dan mengirimnya ke
      `mistral-ocr` via plugin `file-parser` OpenRouter, ditagih ke kunci BYOK (~$0,002/halaman).
      Opsional dengan preview biaya di `ResearchApp.jsx`/`App.jsx`.
      - **Versi `4604ccf` tidak pernah bekerja** — klaim "live-verified Operator Handbook" di catatan
        sebelumnya salah (yang diunggah KATALOG-PENDAS; tak ada OCR). Detektor memutus baris per
        `hasEOL` (pdf.js memberi `hasEOL` per sel tabel) → KATALOG-PENDAS 0, Buku 0, Operator Handbook
        28 halaman (riset: 130 / 3 / 152); OCR membaca `message.content` (tulisan ulang Gemini), bukan
        `annotations` tempat hasil mistral-ocr.
      - **Perbaikan `de75807`:** deteksi per baris visual (`y` per 3 pt); teks OCR dari `annotations`
        dengan `max_tokens: 16`; tag palsu hanya dibuang sebagai deretan akhir halaman non-HTML;
        `$\equiv$` → `=`; PDF dimuat sekali. Uji 26/26 per proyek (sebelumnya 14 gagal); halaman
        terdeteksi pada 5 PDF sama dengan riset.
      - **Bukti live (`npm run desktop`, 06.02 UTC):** KATALOG-PENDAS diunggah ulang dengan OCR
      ...
      - **OCR paralel + coba ulang (sesudah uji live):** unggahan ±130 halaman terasa lama karena halaman dikirim satu per satu. `terapkanOcrHalaman` kini menjalankan hingga 5 halaman serentak (`OCR_SERENTAK`), hasil tetap urut, progres dihitung per halaman selesai; satu halaman gagal → sisa tak dikirim, unggahan dibatalkan. `ocrHalamanPdf` mencoba ulang 429/5xx hingga 3× (1 s, 2 s, atau `Retry-After`); 401 langsung gagal. Uji fetch palsu 33 lolos / 1 gagal (berkas Operator Handbook tak ada) di frontend & mametlite: puncak serentak 5, 12 halaman ±480 ms vs ≥1.200 ms berurutan, 429→ulang→sukses, 429 terus berhenti di 3, halaman gagal → 7/12 terkirim. Build keduanya sukses. **Belum diuji live** — kecepatan nyata dan batas laju OpenRouter belum diukur.
      - **`pdf-lib` tak terbundel di mamet-ecosystem (sejak `4604ccf`):** unggahan di Vercel gagal `Failed to resolve module specifier 'pdf-lib'`. Obfuscator (`stringArray`) menyembunyikan `'pdf-lib'` dari Vite sehingga pustaka tak dibundel; `pdfOcrService.js` belum dikecualikan seperti `documentTextExtractor.js` (Item 69). Tak ketahuan karena uji live di `npm run desktop` (tanpa obfuscator) dan pemeriksaan Vercel hanya mencari teks penanda. mametlite tak terkena (tanpa obfuscator). Perbaikan: `pdfOcrService.js` masuk `exclude` di `frontend/vite.config.js`; build lokal: pdf-lib di chunk `index-BzRsIcXZ.js`, modul 3.110 → 3.305, chunk OCR `import("./index-BzRsIcXZ.js")`, string `"pdf-lib"` 0.
      - **Bukti live di web (06.42 UTC, sesudah deploy `26f3b41`):** KATALOG-PENDAS diunggah ulang di mamet-ecosystem.vercel.app dengan OCR — unggahan OCR pertama yang berhasil di web dan uji live pertama OCR paralel. 813/813 potongan ber-embedding, 518 bertabel, 476 diawali tabel semuanya berjudul kolom, tag palsu/entitas/`$\equiv$`/`<br>` = 0 (desktop 06.02: 519 / 480). Tanpa galat `pdf-lib`, tanpa 429; Owner menilai kecepatan cukup. Bundel live: chunk OCR `ResearchApp-Dr1aXPYr.js` → `import("./index-BzRsIcXZ.js")` (pdf-lib asli), nama `"pdf-lib"` mentah 0.
      - **Status:** ✅ **Selesai & terbukti live** — `npm run desktop` dan mamet-ecosystem.vercel.app (unggahan OCR sungguhan); mametlite: kode + pdf-lib terbundel di Vercel, unggahan OCR di sana belum dicoba. Detail:
    ...
    - **Keterbatasan / belum dikerjakan:**
      ...
      - Jalur PDF: detektor, pemisah halaman (`pdf-lib`), dan mistral-ocr + pembersih terpasang dan terbukti live (Item 76b, `de75807`). OCR paralel terbukti live di web. Yang masih belum: tampilan biaya unggah persisten (ledger), buku penuh sekali kirim, uji mutu jawaban, pemeriksaan salah baca kata OCR, dan unggah ulang Operator Handbook.
