...
15. **Dekomposisi Penuh `engineer.js` (2978 Baris) — [`ADR-0017-engineer-js-decomposition.md`](../adr/ADR-0017-engineer-js-decomposition.md):**
    ...
    - **Progres:** Fase 1 (`SessionArtifact.js`) & Fase 2 (`IntentClassifier.js`, `CapabilityGuard.js`) selesai — `engineer.js` 2978 → **2666 baris**. Fase 2 menemukan koreksi terhadap rencana awal: `_checkCapabilityAndDeclare` ternyata memanggil 3 method lain (bukan pure function seperti diasumsikan), diselesaikan dengan pola dependency-injection (`deps` parameter) tanpa mengubah perilaku. Diverifikasi build + 11 skenario fungsional live + instance Engineer sungguhan tetap sehat. Selanjutnya: Fase 3 (`StaticCodeAnalyzer.js`).
