// UJI PENGAMBILAN POTONGAN (Item 90 Tahap A, 2026-09-17) — alat ukur pencarian dokumen, bukan mutu jawaban.
//
// Pemakaian — dari konsol DevTools `npm run desktop` (mode dev), sudah login:
//   await (await import('/@fs/D:/SLAMET/other/mamet os ecosystem/frontend/node_modules/.uji-rag/uji-pengambilan.js')).uji()
...
async function satuPutaran({ set, governor, supabase, userId, hanya }) {
  const baris = [];
  let kontrol = null;
...
export async function uji({ ulang = 1, hanya = null } = {}) {
  ...
  console.log('[UJI-AMBIL] versi skrip 2 (menyimpan peringkat penuh untuk Tahap B)');
  const putaran = [];
  for (let u = 0; u < ulang; u++) {
    const hasil = await satuPutaran({ set, governor, supabase, userId: session.user.id, hanya });
    putaran.push(hasil);
  ...
  const keluaran = { waktu: new Date().toISOString(), versi_set: set.versi, ambang: AMBANG_SERVER, ambil: AMBIL_SERVER, identik_antar_putaran: sama, putaran };
  const berkas = new Blob([JSON.stringify(keluaran, null, 2)], { type: 'application/json' });
  const a = Object.assign(document.createElement('a'), { href: URL.createObjectURL(berkas), download: `uji-pengambilan-${keluaran.waktu.replace(/[:.]/g, '-')}.json` });
